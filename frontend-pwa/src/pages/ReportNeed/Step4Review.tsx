...
import { Mic, Type, Image as ImageIcon, MapPin, Tag, ChevronLeft } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="bg-white px-5 py-4 border-b border-gray-200 sticky top-0 z-10 shadow-sm flex items-center gap-3">
        <button onClick={() => navigate(-1)} className="p-2 -ml-2 rounded-full hover:bg-gray-100 text-gray-700"><ChevronLeft size={24} /></button>
        <h1 className="text-xl font-bold text-gray-900">{t('report.step4.title')}</h1>
      </div>
      ...
      <div className="flex-1 p-5 pb-24 space-y-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            ...
            <button onClick={() => navigate('/report/step1')} className="text-sm font-medium text-teal-600">Edit</button>
          </div>
          <div className="p-4">
            {draft.audioUrl && (
            ...
            {draft.photoUrl && (
              <div className="mt-4 rounded-lg overflow-hidden border border-gray-200">
                <img src={draft.photoUrl} alt="Reported issue" className="w-full h-48 object-cover" />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            ...
            <button onClick={() => navigate('/report/step2')} className="text-sm font-medium text-teal-600">Edit</button>
          </div>
          <div className="p-4">
            {draft.location ? (
              <div>
                {draft.location.localityName && <p className="font-medium text-gray-900">{draft.location.localityName}</p>}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            ...
            <button onClick={() => navigate('/report/step3')} className="text-sm font-medium text-teal-600">Edit</button>
          </div>
          <div className="p-4">
            {draft.categoryHint ? (
              <p className="font-medium text-gray-900 capitalize">{draft.categoryHint.replace('_', ' ')}</p>
            ) : (
              <p className="text-gray-500 italic">Not selected (Civora will auto-identify)</p>
            )}
          </div>
        </div>
      </div>
