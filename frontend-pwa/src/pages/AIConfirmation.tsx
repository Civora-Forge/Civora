...
import { MapPin, Check, ChevronLeft, AlertCircle, BarChart3, Layers } from 'lucide-react';
...
export const AIConfirmation = () => {
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="bg-red-50 p-4 rounded-xl flex items-start gap-3 border border-red-100 max-w-sm">
            ...
            <div>
              <p className="text-sm text-red-800 font-medium">No response data available.</p>
              <button onClick={() => navigate('/report/step4')} className="mt-2 text-sm text-red-600 underline">
  ...
  const categoryDisplay = backendResponse.finalCategory
    || draft.categoryHint
    || "other";

  const projectTitle = backendResponse.projectTitle
    || draft.text
    || "Civic improvement need";

  const locationName = draft.location?.localityName
      ...
      <div className="flex-1 p-5 pb-32 space-y-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-5 border-b border-gray-100">
            <h2 className="text-lg font-bold text-gray-900 leading-tight mb-2">{projectTitle}</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-gray-500">Priority</span>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-gray-500">Cluster</span>
            </div>
            <p className="text-sm font-mono text-gray-700 truncate">
              {backendResponse.clusterId || "N/A"}
            </p>
            {backendResponse.clusterSummary && (
              <p className="text-xs text-gray-500 mt-1 truncate" title={backendResponse.clusterSummary}>
                {backendResponse.clusterSummary}
              </p>
            )}
          </div>
        ...
        {backendResponse.recommendedDepartment && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <span className="text-xs font-bold uppercase tracking-wider text-gray-500">Recommended Department</span>
            <p className="text-sm text-gray-800 mt-1">{backendResponse.recommendedDepartment}</p>
          </div>
        )}

        {backendResponse.explanation && backendResponse.explanation.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            <span className="text-xs font-bold uppercase tracking-wider text-gray-500">Priority Reasoning</span>
