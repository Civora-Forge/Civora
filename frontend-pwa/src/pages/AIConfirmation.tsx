...
export const AIConfirmation = () => {
  ...
  const interpretation = {
    title: draft.text ? (draft.text.length > 40 ? draft.text.substring(0, 40) + '...' : draft.text) : 'Need reported via Audio/Photo',
    summary: draft.text || 'The citizen provided audio or visual evidence of a local development need.',
    category: draft.categoryHint || 'other',
    locationName: draft.location?.localityName || `Lat: ${draft.location?.latitude.toFixed(3)}, Lng: ${draft.location?.longitude.toFixed(3)}`
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      setError("Civora couldn't process your request right now. Please try again or wait for connectivity.");
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="bg-white px-5 py-4 border-b border-gray-200 sticky top-0 z-10 shadow-sm flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-gray-900">Is this what you meant?</h1>
      </div>

      <div className="flex-1 p-5 pb-32">
        <p className="text-gray-600 mb-6">Please check how Civora understood your request.</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-5 border-b border-gray-100">
            <h2 className="text-lg font-bold text-gray-900 leading-tight mb-2">{interpretation.title}</h2>
            <div className="flex gap-2 items-center text-sm">
              ...
              <span className="flex items-center text-gray-600">
                <MapPin size={14} className="mr-1" /> {interpretation.locationName}
              </span>
            </div>
          </div>
          <div className="p-5 bg-gray-50">
            <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Summary</h3>
            <p className="text-gray-700 text-sm leading-relaxed">{interpretation.summary}</p>
          </div>
